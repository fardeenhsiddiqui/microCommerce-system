...
public class RabbitMQConfig {
    ...
    public Queue reStockQueue(){
        return new Queue(RabbitMQConstants.PRODUCT_RESTOCK);
    }

    // Product Exchange
    @Bean
    public DirectExchange productExchange(){
        return new DirectExchange(RabbitMQConstants.PRODUCT_EXCHANGE);
    }
    ...
    public Binding reStockBinding(Queue reStockQueue, DirectExchange exchange){
        return BindingBuilder.bind(reStockQueue)
                .to(exchange)
                .with(RabbitMQConstants.PRODUCT_RESTOCK_KEY);
    }
